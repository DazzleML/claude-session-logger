...
- **New `cclogger/paths.py`**: single owner of the Claude-dir layout (`claude_dir()` + accessors for each subtree). The standalone `rename_session.py` carries a documented mirror of the resolver (it deliberately avoids importing the cclogger package, the same pattern csb's backup-hook uses). 7 new tests (`tests/one-offs/test_paths.py`); 378 total.
